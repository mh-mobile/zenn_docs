...
前回実行したコードと同じ範囲のコードをサイド実行した場合、前回の実行結果を削除し、新しい実行結果で上書きします。

特に Rust Analyzer の `dbg!` マクロを使用したデバッグ時に便利です。`dbg!` の出力をインラインで確認しながら、コードを修正して再実行することで、効率的なデバッグが可能になります。

![](https://storage.googleapis.com/zenn-user-upload/17217588bcde-20241224.gif)

:::caution
`Rust Analyzer` の **ポストフィクス補完** 機能を活用することで、コーディング効率をさらに向上させることができます。
...
- Visual Studio Code
- [Jupyter VSCode 拡張機能](https://marketplace.visualstudio.com/items?itemName=ms-toolsai.jupyter)
...
- 特に Ruby でのリッチコンテンツの出力には現在対応していません
...
4. **Result Renderer**
   - VSCode の Comment API を使用した実行結果のインライン表示
